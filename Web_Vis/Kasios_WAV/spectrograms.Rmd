...
```{r}
sound <- loadsound(filename = "4.wav")
```

```{r}
file <- '4.wav'
title <- substr(file, 1, nchar(file) - 4)
title <- paste(title, " Spectrogram")
spectrogram(sound = sound, nlevels=10, dynamicrange = 100, colors = topo.colors(100), maintitle = title, maxfreq = 10000)
...
for (file in files){
  ...
  spectrogram(sound = sound, nlevels=10, dynamicrange = 100, maxfreq = 10000, colors = topo.colors(100))
}
